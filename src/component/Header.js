...
const Header = () => {
  ...
  const navLinks = [
  ...
  
    {
      ...
      submenu: true,
      dropdown: [
        { name: " Principle's Message", link: "/chairman" },
        { name: " Board Of Trustees", link: "/clientele" },
        { name: "  Mission & Vision", link: "/mission&vision" },
        { name: "   History", link: "/history" },
        { name: "    Accreditation", link: "/Accreditation" },
        { name: " School Infrasture   ", link: "/schoolinfrasture" },
      ]
    },
...

    {
      ...
      submenu: true,
      dropdown: [
        { name: "  Teaching Faculty ", link: "/nursing" },
  ...
  <div className="flex   bg-red-600 h-[25px] text-white justify-center sm:justify-center md:justify-end sm:gap-x-[10px] gap-x-[10px]  md:gap-x-[30px]">
    <Link to='/login' className="text-[15px] no-underline text-white cursor-pointer hover:text-green-500 hover:text-underline"> Sign In </Link>
    <Link to='/register' className="text-[15px] no-underline text-white cursor-pointer hover:text-green-500  ">| &nbsp;Apply Now </Link>
    <Link to='/' className="text-[15px] mr-[4rem]  no-underline text-white cursor-pointer hover:text-green-500 "> |&nbsp; Career Opportunities</Link>
  </div>
 ...
 <div className={`sticky top-0 z-[1000] text-black Georgian bg-white px-4 md:px-6 w-full border-b-2 border-red-700 `}>
...
<div className={`h-1 flex flex-col ml-10 md:flex-row justify-between  md:text-[17px]   md:pt-[30px] md:mt-2 items-center font-bold md:px-7 text-sm ${showShadow ? 'shadow-2xl' : ''}`} >
            ...
            <div className="hidden lg:inline Georgian">
              <ul className="flex justify-center hover:cursor-pointer ">
                {navLinks.map((li, index) => (
                  ...
                  >
                    <Link to={li.url} className="flex text-black no-underline " >
                      {li.text}
                      {li.submenu ? (
